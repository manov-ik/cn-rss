...
describe("dataAgent", () => {
  ...
  describe("Legacy Tauri command functions (now HTTP POST)", () => {
    describe("createFolder", () => {
      it("should create folder via HTTP request", async () => {
        const mockResult = 1;
        (request.post as any).mockResolvedValue({ data: mockResult });

        const result = await dataAgent.createFolder("Test Folder");

        expect(request.post).toHaveBeenCalledWith("create_folder", {
          name: "Test Folder",
        });
    ...
    describe("updateFolder", () => {
      it("should update folder via HTTP request", async () => {
        const mockResult = 1;
        (request.post as any).mockResolvedValue({ data: mockResult });

        const result = await dataAgent.updateFolder("folder-uuid", "New Name");

        expect(request.post).toHaveBeenCalledWith("update_folder", {
          uuid: "folder-uuid",
          name: "New Name",
        });
        expect(result).toBe(mockResult);
    ...
    describe("subscribeFeed", () => {
      it("should subscribe to feed via HTTP request", async () => {
        ...
        expect(request.post).toHaveBeenCalledWith("add_feed", {
          url: "https://example.com/feed",
        });
        expect(result).toEqual(mockResult);
      });
    ...
    describe("updateThreads", () => {
      it("should update threads via HTTP request", async () => {
        const mockResult = { success: true };
        (request.post as any).mockResolvedValue({ data: mockResult });

        const result = await dataAgent.updateThreads(4);

        expect(request.post).toHaveBeenCalledWith("update_threads", {
          threads: 4,
        });
        expect(result).toEqual(mockResult);
      });
